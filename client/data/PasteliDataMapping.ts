...
interface PestaliItem {
   ...
   */
  export function parsePestaliData(
    content_data: string
  ): { pestaliData: PestaliItem[]; sources: string[] } {
    // 1. Find the "pestali_analysis:" token
    ...
    let analysisPart = content_data.substring(startIndex + key.length).trim();
    ...
    if (braceStart === -1 || braceEnd === -1 || braceEnd <= braceStart) {
      throw new Error(`Could not find valid JSON braces for 'pestali_analysis'.`);
    }
